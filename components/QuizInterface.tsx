...
import { Check, ArrowRight, Clock, Heart, Flame, Sparkles, Lightbulb, Volume2, Mic, Bot } from 'lucide-react';
               ...
               {aiExplanation && (
                 ...
                 >
                   <div className="flex items-start gap-3">
                     ...
                     <p className="text-sm text-slate-700 italic">"{aiExplanation}"</p>
                   </div>
                 </motion.div>
               )}
...
export const QuizInterface: React.FC<QuizInterfaceProps> = ({ questions, mode, onComplete }) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Check if currentQuestion exists and has options before checking length
      if (currentQuestion && currentQuestion.options && 
          keyMap[e.key] !== undefined && keyMap[e.key] < currentQuestion.options.length) {
  ...
  }, [isAnswered, currentQuestion, handleOptionSelect]);
    ...
    <div className="max-w-3xl mx-auto px-4">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2">
           {/* Sound Indicator (Visual only, to show it's enabled) */}
